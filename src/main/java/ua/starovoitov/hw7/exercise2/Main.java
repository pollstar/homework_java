 ...
 *
 * Создать enum Manufacturer, задать несколько значений, enum также должен содержать поле russianName
 * которое содержит текст. Изменить класс Car на использование Manufacturer.
 * Вывод Car в консоль должен печатать название производителя на русском
 *
 * Создать класс Engine который содержит поля power и type. Изменить класс Car на использование Engine.
 *
 * Создать класс Garage который содержит один статический метод checkСar - в качестве параметра принимает объект Car
 * проверяет состояние бензина
 * проверяет мощность двигателя, что она больше 200
 * если оба условия успешно пройдены - пишет в консоль результат
 * если что-то не пройдено - пишет в консоль какая проверка не пройдена и почему
 */
public class Main {
